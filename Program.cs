...
class Program {
	static void Main() {
		...
		using (var connection = new SqliteConnection(connectionString)) {
			...
			// Create a table
			var command = new SqliteCommand("CREATE TABLE data (id INTEGER PRIMARY KEY AUTOINCREMENT, value TEXT)", connection);
			...
			using (var transaction = connection.BeginTransaction()) {
				for (int i = 0; i < insertCount; i++) {
					var insertCommand = new SqliteCommand("INSERT INTO data (value) VALUES (@value)", connection, transaction);
